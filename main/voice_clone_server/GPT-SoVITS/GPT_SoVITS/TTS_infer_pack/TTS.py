...
class TTS:
    ...
    def init_bert_weights(self, base_path: str):
        import os as _os
        abs_path = _os.path.abspath(base_path)
        print(f"Loading BERT weights from {abs_path}")
        self.bert_tokenizer = AutoTokenizer.from_pretrained(abs_path)
        self.bert_model = AutoModelForMaskedLM.from_pretrained(abs_path)
    ...
    def _get_ref_spec(self, ref_audio_path):
        ...
        if raw_audio_np.ndim == 1:
            raw_audio_np = raw_audio_np[np.newaxis, :]
        else:
